...
/// Risk 1 — Double bottom-nav. OFF: the poker module renders its OWN bottom bar
/// (like flutter-poker's NavigationBarScreen) → two stacked bars. ON: suppressed.
final fixDoubleBarProvider =
...
/// nested ProviderScope that overrides [pokerTabLabelProvider], so poker's
/// "rename" writes never reach the navbar. ON: no shadow → writes propagate.
final fixShadowScopeProvider =
    NotifierProvider<BoolNotifier, bool>(() => BoolNotifier(false));

/// Risk 3 — Bar visibility isn't automatic. OFF: opening a go_router fullscreen
/// route leaves the auto_route bar visible. ON: poker wires the route lifecycle
/// to navBarVisibleProvider so the bar hides and restores.
